...
const updateFoodItem = async (req, res) => {
    ...
    try {
        ...
        if (req.file && req.file.path) {
            ...
            if (currentImgId) {
                const deleteOldImageResult = await deleteImage(
                    currentImgId,
                    foodUploadFolder
                );
                ...
                if (
                    deleteOldImageResult?.deleted_counts[`${currentImgPath}`]
                        ?.original > 0
                ) {
                    console.log(
                        `Ole image ID: ${currentImgId} was deleted successfully.`
                    );
                } else {
                    console.error(
                        `Failed to delete old image ID: ${currentImgId}.`
                    );
                }
            }
            ...
            if (uploadResult.original_filename) {
                const deleteOriginalFileNameResult =
                    await deleteOriginalFileName(
                ...
                if (
                    ...
                    console.log(
                        `Image original file name ${uploadResult.original_filename} was deleted successfully.`
                    );
                } else {
                    console.error(
                        `Failed to delete image riginal filename ${uploadResult.original_filename}.`
                    );
                }
            }
        } else {
    ...
    try {
        ...
        if (foodImage) {
            ...
            if (currentImgId) {
                ...
                if (
                    deleteImageResult?.deleted_counts?.[`${currentImgPath}`]
                        ?.original > 0
                ) {
                    console.log(
                        `Image ${currentImgId} was deleted successfully.`
                    );
                } else {
