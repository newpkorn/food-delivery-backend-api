...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        const { password: user_password, ...userInfo } = user.toObject();
...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ success: true, data: userInfo, token });
    } catch (error) {
...
const updateUserInfo = async (req, res) => {
    ...
    if (id !== currentUserLoginId && req.user?.role !== 'admin') {
        return res
            .status(401)
        ...
        if (!isPasswordValid) {
            if (image_filename) {
                try {
                    ...
                    if (
                        !deleteResult ||
                        deleteResult?.deleted_counts?.[`${originalFileName}`]
                            ?.original === 0
                    ) {
                        console.error(
            ...
            if (currentImage && typeof currentImage === 'string') {
                ...
                if (currentImgId) {
                    ...
                    if (
                        ...
                        console.error(
                            `Failed to delete image: ${currentImgId}`
                        );
                    }
                }
            }
